...
pub fn buildRemovedAdjEmpty(source_adj: types.NodeAdj) types.NodeAdj {
    ...
    removed_adj.segment_count_fwd = 0;
    removed_adj.first_segment_fwd = 0;
    removed_adj.first_block_rev = 0;
    removed_adj.block_count_rev = 0;
    removed_adj.segment_count_rev = 0;
    removed_adj.first_segment_rev = 0;
...
pub fn publishRelatedNodeUpdates(graph: *graph_core.GraphCore, related_nodes: []const remove_types.RelatedNode) remove_types.RemoveCounts {
    ...
    for (related_nodes) |related| {
        ...
        const state = related.node_publication.loadPublicationState();
        ...
        const node_adjacency_buffers = page_ops.nodeAdjacencyBuffersAt(graph, node_id);
        if (related.fwd_degree_delta > 0 and related.rev_degree_delta > 0) {
            _ = common.publishStateBothDeltaNoFlip(related.node_publication, node_adjacency_buffers, .{
        ...
        if (related.fwd_degree_delta > 0) {
            _ = common.publishStateFwdDeltaNoFlip(related.node_publication, node_adjacency_buffers, true, -@as(i23, @intCast(related.fwd_degree_delta)));
        }
        if (related.rev_degree_delta > 0) {
            _ = common.publishStateRevDeltaNoFlip(related.node_publication, node_adjacency_buffers, true, -@as(i23, @intCast(related.rev_degree_delta)));
        }
    }
    return counts;
}
