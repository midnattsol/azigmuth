...
fn previousBlockInSide(graph: *const graph_core.GraphCore, side_view: types.SideAdj, block_idx: u32) ?u32 {
    const total_segments = side_segments.segmentCount(side_view);
    var previous_segment_last: ?u32 = null;
    var segment_idx: u16 = 0;
    while (segment_idx < total_segments) : (segment_idx += 1) {
        const segment = side_segments.segmentAt(graph, side_view, segment_idx) orelse return null;
        if (block_idx >= segment.start and block_idx < segment.start + segment.count) {
            if (block_idx > segment.start) return block_idx - 1;
            return previous_segment_last;
        }
        previous_segment_last = segment.start + segment.count - 1;
    }
    return null;
}
...
fn addEdgeImpl(
    ...
    const source_staging = node_access.stagingFwd(graph, source, endpoints.source_state);
    const destination_staging = node_access.stagingRev(graph, destination, endpoints.destination_state);

    if (!graph.multigraph_enabled) {
        const source_sorted = endpoints.source_buffers.publishedFwdSortedFromState(endpoints.source_state);
        if (try adjacency.hasEdgeInSideAdjChecked(graph, source_published_side, destination.index, source_sorted)) {
            return error.EdgeAlreadyExists;
        }
    }

    if (node_access.publishedFwdDegreeFromStateAtConst(graph, source, endpoints.source_state) >= constants.MAX_DEGREE_PER_SIDE) return error.DegreeLimitReached;
    if (node_access.publishedRevDegreeFromStateAtConst(graph, destination, endpoints.destination_state) >= constants.MAX_DEGREE_PER_SIDE) return error.DegreeLimitReached;

    var scratch = common.MutationScratch{};
    defer scratch.deinit(graph.allocator);
    defer scratch.cleanup(graph);

    const edge_id = if (graph.multigraph_enabled) try endpoints.claims.source_mutation_control.nextEdgeId() else types.EdgeId{ .local = 0 };
    const prop_row: u32 = if (graph.edge_properties_enabled) try scratch.allocPropRow(graph) else 0;

    const old_forward_segments = shared.OldSegmentSlots.captureSide(&source_published_side);
    const old_reverse_segments = shared.OldSegmentSlots.captureSide(&destination_published_side);
    ...
    if (add_build.canUseTinyFwdSide(graph, source_published_side)) {
        source_staging.* = try add_build.buildForwardTinyOrPromoted(graph, source_published_side, destination, relation, flags, edge_id.local, prop_row, &scratch);
        // Tiny slots and single promoted blocks are trivially globally sorted.
        fwd_sorted_after = true;
    } else {
        const fwd_sorted_before = endpoints.source_buffers.publishedFwdSortedFromState(endpoints.source_state);
        forward_prepared = try prepareAppendBlockSide(graph, source_staging, .fwd, &scratch);
        try local_repair.ensureTailCowSegmentConstraint(graph, source_staging, forward_prepared);
    ...
    if (add_build.canUseTinyRevSide(destination_published_side)) {
        destination_staging.* = try add_build.buildReverseTinyOrPromoted(graph, destination_published_side, source, &scratch);
        rev_sorted_after = true;
    } else {
        const rev_sorted_before = endpoints.destination_buffers.publishedRevSortedFromState(endpoints.destination_state);
        reverse_prepared = try prepareAppendBlockSide(graph, destination_staging, .rev, &scratch);
        try local_repair.ensureTailCowSegmentConstraint(graph, destination_staging, reverse_prepared);
    ...
    try add_finalize.finalizeAdded(
        ...
        reverse_prepared,
        reverse_applied,
        old_forward_segments,
        old_reverse_segments,
        fwd_sorted_after,
        rev_sorted_after,
    );
    ...
    if (node_adjacency_buffers_mod.NodeAdjacencyBuffers.isTiny(&source_published_side)) {
        rcu.retireTinySlot(graph, source_published_side.first_block, .fwd);
    }
    if (node_adjacency_buffers_mod.NodeAdjacencyBuffers.isTiny(&destination_published_side)) {
