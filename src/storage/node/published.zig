...
comptime {
    if (@sizeOf(NodePublished) != (@sizeOf([2]types.SideAdj) * 2) + (@sizeOf([2]u32) * 2)) {
        @compileError("NodePublished must cover the two published side buffers plus degree sidecars");
    }
}
