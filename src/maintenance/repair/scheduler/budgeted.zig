...
const scheduler_apply = @import("apply.zig");
const scheduler_scan = @import("scan.zig");
...
fn repairBudgetedWork(
    ...
    while (total_compacted < max_nodes) {
        const node_idx = scheduler_scan.nextRepairDebtNode(graph, processed_nodes) orelse break;
...
pub fn repairBudgeted(graph: *graph_core.GraphCore, max_nodes: usize) !usize {
    ...
    const total_compacted = try repairBudgetedWork(graph, max_nodes, &processed_nodes);
...
pub fn flushRepairs(graph: *graph_core.GraphCore) !types.RepairFlushSummary {
    ...
    const repaired_nodes = try repairBudgetedWork(graph, node_count, &processed_nodes);
    ...
    return .{
        ...
        .pass_count = if (node_count == 0) 0 else 1,
