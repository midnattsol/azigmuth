...
pub const Graph = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) !Graph {
        ...
        return .{ .core = state_value };
    }

    pub fn deinit(self: *Graph) void {
        const alloc = self.core.allocator;
        for (self.core.node_pages.items) |page| alloc.free(page);
        for (self.core.edge_blocks_fwd.items) |page| alloc.free(page);
        for (self.core.edge_blocks_rev.items) |page| alloc.free(page);
        for (self.core.edge_block_groups.items) |page| alloc.free(page);

        self.core.node_pages.deinit(alloc);
        self.core.edge_blocks_fwd.deinit(alloc);
        self.core.edge_blocks_rev.deinit(alloc);
        self.core.edge_block_groups.deinit(alloc);

        self.core.free_blocks_fwd.deinit(alloc);
        self.core.free_blocks_rev.deinit(alloc);
        self.core.free_groups.deinit(alloc);
        self.core.retired_blocks_fwd.deinit(alloc);
        self.core.retired_blocks_rev.deinit(alloc);
        self.core.repair_fwd.deinit(alloc);
        self.core.repair_rev.deinit(alloc);
    }

    // ── Node API ──────────────────────────────────────────────────────

    pub fn addNode(self: *Graph) !types.NodeId {
        const index = self.core.node_count;
        const page = page_ops.pageOf(index, constants.NODES_PER_PAGE);

        if (page == self.core.node_pages.items.len) {
            const new_page = try self.core.allocator.alloc(types.NodeBuffer, constants.NODES_PER_PAGE);
            @memset(new_page, std.mem.zeroes(types.NodeBuffer));
            try self.core.node_pages.append(self.core.allocator, new_page);
        }
    ...
    pub fn nodeCount(self: *const Graph) usize {
        return self.core.node_count;
    }

    pub fn edgeCount(self: *const Graph) u64 {
        return self.core.edge_count.load(.acquire);
    }

    pub fn hasNode(self: *const Graph, id: types.NodeId) bool {
        return id.index < self.core.node_count;
    }
...
test "graph: removeEdge rejects non-tail occupancy below threshold without publishing" {
    ...
    // RFC §3.6 requires synchronous repair or error.RepairRequired.
...
test "graph: repairNode compacts under-full adjacent blocks" {
    ...
    var first_block_edges = page_ops.edgeBlockAt(&graph.core, block0, .fwd);
    var second_block_edges = page_ops.edgeBlockAt(&graph.core, block1, .fwd);
    for (0..47) |i| {
        first_block_edges.edges[i] = types.Edge{ .dest = @intCast(i + 1), .relation = 0, .flags = @bitCast(@as(u16, 0)) };
    }
    first_block_edges.mask = constants.denseMask(47);

    for (0..36) |i| {
        second_block_edges.edges[i] = types.Edge{ .dest = @intCast(i + 48), .relation = 0, .flags = @bitCast(@as(u16, 0)) };
    }
    ...
    graph.core.edge_count.store(83, .release);

    const repair_mod = @import("repair.zig");
    const compacted = try repair_mod.repairNode(&graph.core, src, .fwd);
...
test "graph: neighbor iterator deinit releases RCU reader count" {
    ...
    try testing.expectEqual(@as(u32, 0), graph.core.active_readers.load(.acquire));

    {
        var it = try graph.neighbors(a);
        // After creating iterator, active_readers should be 1
        try testing.expectEqual(@as(u32, 1), graph.core.active_readers.load(.acquire));
        _ = it.next();
        it.deinit();
    }
...
test "graph: reader guard remains active until iterator deinit" {
    ...
    const start_readers = graph.core.active_readers.load(.acquire);
    var it = try graph.neighbors(a);

    const after_readers = graph.core.active_readers.load(.acquire);
...
test "graph: debugValidate detects forward entry without reverse entry" {
    ...
    var fwd = page_ops.edgeBlockAt(&graph.core, block, .fwd);
    fwd.edges[0] = types.Edge{ .dest = b.index, .relation = 0, .flags = @bitCast(@as(u16, 0)) };
    ...
    graph.core.edge_count.store(1, .release);
...
test "graph: debugValidate detects reverse entry without forward entry" {
    ...
    var rev = page_ops.edgeBlockAt(&graph.core, block, .rev);
...
test "graph: rejected non-tail removal does not allocate or retire blocks" {
    ...
    const fwd_blocks_before = graph.core.block_fwd_count;
    const rev_blocks_before = graph.core.block_rev_count;
    const retired_fwd_before = graph.core.retired_blocks_fwd.items.len;
    const retired_rev_before = graph.core.retired_blocks_rev.items.len;
    const free_fwd_before = graph.core.free_blocks_fwd.items.len;
    const free_rev_before = graph.core.free_blocks_rev.items.len;

    try testing.expectError(error.RepairRequired, graph.removeEdge(src, targets[16]));

    try testing.expectEqual(fwd_blocks_before, graph.core.block_fwd_count);
    try testing.expectEqual(rev_blocks_before, graph.core.block_rev_count);
    try testing.expectEqual(retired_fwd_before, graph.core.retired_blocks_fwd.items.len);
    try testing.expectEqual(retired_rev_before, graph.core.retired_blocks_rev.items.len);
    try testing.expectEqual(free_fwd_before, graph.core.free_blocks_fwd.items.len);
    try testing.expectEqual(free_rev_before, graph.core.free_blocks_rev.items.len);
    try graph.validate();
}

test "graph: retired blocks are reclaimed only after readers exit" {
    var graph = try Graph.init(testing.allocator);
    defer graph.deinit();
    ...
    try testing.expect(graph.core.retired_blocks_fwd.items.len > 0);
    try testing.expectEqual(@as(usize, 0), graph.core.free_blocks_fwd.items.len);
    ...
    try testing.expectEqual(@as(usize, 0), graph.core.retired_blocks_fwd.items.len);
    try testing.expect(graph.core.free_blocks_fwd.items.len > 0);
}
...
test "graph: repairBudgeted processes queued repair debt" {
    ...
    node.storePublishedAdjIndex(0);
    graph.core.edge_count.store(83, .release);
    try graph.core.repair_fwd.append(graph.core.allocator, src.index);

    const repair_mod = @import("repair.zig");
    const compacted = try repair_mod.repairBudgeted(&graph.core, 1);
    try testing.expect(compacted > 0);
    try testing.expectEqual(@as(usize, 0), graph.core.repair_fwd.items.len);
    try graph.validate();
    try testing.expectEqual(@as(usize, 83), try graph.outDegree(src));
}
