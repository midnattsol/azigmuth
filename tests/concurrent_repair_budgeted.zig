...
test "concurrent repair: repairBudgeted + addEdge in parallel does not crash or corrupt" {
    ...
    const repairer_thread = try std.Thread.spawn(.{}, struct {
    ...
    const mutator_thread = try std.Thread.spawn(.{}, struct {
        fn run(g: *graph_mod.Graph, stop_ptr: *std.atomic.Value(bool)) void {
            var spin: usize = 0;
            while (!stop_ptr.load(.acquire) and spin < 200_000_000) : (spin += 1) {
                const a_idx: u32 = @intCast(spin % 30);
                const b_idx: u32 = @intCast((spin + 1) % 30);
                if (a_idx != b_idx) {
                    _ = g.addEdge(.{ .index = a_idx }, .{ .index = b_idx }, 0, 0) catch {};
                }
                std.atomic.spinLoopHint();
            }
        }
    ...
    // Final validation must not crash and should show no structural corruption.
    const violations = try graph.debugValidate(testing.allocator);
    defer testing.allocator.free(violations);

    // layout debt / tombstone flags are acceptable; structural corruption is not.
    for (violations) |v| {
        switch (v) {
            .block_double_owned,
