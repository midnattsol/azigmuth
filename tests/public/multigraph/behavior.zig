...
test "multigraph: outDegree reflects duplicate edges" {
    ...
    try g.addEdge(a, b, 0, .{});
    try g.addEdge(a, b, 1, .{});
    try testing.expectEqual(@as(usize, 2), try snapshot_support.outDegree(g, a, testing.allocator));
    try testing.expectEqual(@as(usize, 2), try snapshot_support.inDegree(g, b, testing.allocator));
}
...
test "multigraph: removeEdgeWithId removes specific edge" {
    ...
    try testing.expectEqual(@as(usize, 1), try snapshot_support.outDegree(g, a, testing.allocator));
    try testing.expectEqual(@as(usize, 1), try snapshot_support.inDegree(g, b, testing.allocator));
...
test "multigraph: removeEdge removes all duplicates" {
    ...
    try testing.expectEqual(@as(usize, 0), try snapshot_support.outDegree(g, a, testing.allocator));
    try testing.expectEqual(@as(usize, 0), try snapshot_support.inDegree(g, b, testing.allocator));
}
...
test "multigraph: outEdges exposes EdgeRef with IDs" {
    ...
    var it = try snapshot_support.outEdges(g, a, testing.allocator);
    defer it.deinit();
    const ref1 = it.next().?;
    const ref2 = it.next().?;
    try testing.expect(it.next() == null);
...
test "multigraph: outEdges orders duplicate destinations by EdgeId and preserves metadata" {
    ...
    var it = try snapshot_support.outEdges(g, source, testing.allocator);
    defer it.deinit();
...
test "multigraph: removeEdge preserves grouped reverse entries after duplicates" {
    ...
    try testing.expectEqual(@as(usize, 82), try snapshot_support.inDegree(g, destination, testing.allocator));
    ...
    try testing.expectEqual(@as(usize, 80), try snapshot_support.inDegree(g, destination, testing.allocator));
    try g.validate();
}
...
test "multigraph: repair preserves multiblock duplicate EdgeIds" {
    ...
    try testing.expectEqual(@as(usize, 70), try snapshot_support.outDegree(g, source, testing.allocator));
    try testing.expectEqual(@as(usize, 70), try snapshot_support.inDegree(g, destination, testing.allocator));
    try g.validate();
}
...
test "multigraph: removeNode with self-edge duplicates" {
    ...
    try g.addEdge(a, a, 1, .{});
    try testing.expectEqual(@as(usize, 2), try snapshot_support.outDegree(g, a, testing.allocator));
    try testing.expectEqual(@as(usize, 2), try snapshot_support.inDegree(g, a, testing.allocator));
