//! Coverage for adjacency run helpers (tailRun, forEachRun) and the
//! forward destination-match counter, over both contiguous and grouped
//! side layouts.
...
const runs = graph_mod.adjacency_runs_mod;
...
test "runs: tailRun on contiguous, grouped, and empty layouts" {
    var graph = try graph_mod.Graph.init(testing.allocator);
    defer graph.deinit();

    const fixture = try setUpBlocks(&graph);

    const contiguous = types.SideAdj{ .first_block = fixture.blk0, .block_count = 2, .group_count = 0, .first_group = 0 };
    const contiguous_tail = runs.tailRun(&graph.graph, contiguous) orelse return error.TestExpectedEqual;
    try testing.expectEqual(fixture.blk0, contiguous_tail.start);
    try testing.expectEqual(@as(u32, 2), contiguous_tail.count);

    const grouped = types.SideAdj{ .first_block = fixture.blk0, .block_count = 2, .group_count = 2, .first_group = fixture.g0 };
    const grouped_tail = runs.tailRun(&graph.graph, grouped) orelse return error.TestExpectedEqual;
    try testing.expectEqual(fixture.blk1, grouped_tail.start);
    try testing.expectEqual(@as(u32, 1), grouped_tail.count);

    const empty = types.SideAdj{ .first_block = 0, .block_count = 0, .group_count = 0, .first_group = 0 };
    try testing.expectEqual(@as(?runs.RunDesc, null), runs.tailRun(&graph.graph, empty));
}
...
fn walkRuns(graph: *const graph_mod.GraphCore, side_adj: types.SideAdj) !RunWalk {
    var walk = RunWalk{};
    try runs.forEachRun(graph, side_adj, &walk, struct {
        fn cb(_: anytype, ctx: *RunWalk, run: runs.RunDesc, is_last: bool) !void {
            ctx.runs_seen += 1;
            ctx.blocks_seen += run.count;
...
test "runs: forEachRun visits one contiguous run or every group" {
    var graph = try graph_mod.Graph.init(testing.allocator);
    defer graph.deinit();
    ...
    const contiguous = types.SideAdj{ .first_block = fixture.blk0, .block_count = 2, .group_count = 0, .first_group = 0 };
    const contiguous_walk = try walkRuns(&graph.graph, contiguous);
    try testing.expectEqual(@as(usize, 1), contiguous_walk.runs_seen);
    ...
    const grouped = types.SideAdj{ .first_block = fixture.blk0, .block_count = 2, .group_count = 2, .first_group = fixture.g0 };
    const grouped_walk = try walkRuns(&graph.graph, grouped);
    try testing.expectEqual(@as(usize, 2), grouped_walk.runs_seen);
    try testing.expectEqual(@as(u32, 2), grouped_walk.blocks_seen);
    try testing.expectEqual(@as(usize, 1), grouped_walk.last_flags_seen);

    const empty = types.SideAdj{ .first_block = 0, .block_count = 0, .group_count = 0, .first_group = 0 };
    const empty_walk = try walkRuns(&graph.graph, empty);
    try testing.expectEqual(@as(usize, 0), empty_walk.runs_seen);
}

test "adjacency: countForwardDestinationMatches over contiguous and grouped layouts" {
    var graph = try graph_mod.Graph.init(testing.allocator);
    defer graph.deinit();

    const fixture = try setUpBlocks(&graph);

    // Contiguous span of both blocks: the target lives once in each.
    ...
    // Invalid layout (span past the allocated pool) degrades to zero matches.
