...
test "node pages: a node that lives in the second page still answers neighbors correctly" {
    ...
    const list = try graph_mod.materializeConsuming(&iterator, testing.allocator);
    ...
    const incoming_list = try graph_mod.materializeConsuming(&incoming, testing.allocator);
