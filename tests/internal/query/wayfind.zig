...
test "exec: expand one hop equals the snapshot's neighbor sets" {
    ...
    for (fixture.nodes) |node| {
        ...
        var result = try exec.segment(plan, &snapshot.view, ctx, .{ .sets = &.{&.{node.index}} });
        defer result.deinit(testing.allocator);
...
test "exec: expand closure equals bfs reachability" {
    ...
    var closed = try exec.segment(with_start, &snapshot.view, ctx, .{});
    ...
    var reachable = try exec.segment(without_start, &snapshot.view, ctx, .{});
...
test "exec: hop ranges respect min and max levels" {
    ...
    var mid = try exec.segment(two_three, &snapshot.view, ctx, .{});
    try expectIds(&mid, &.{ nodes[2].index, nodes[3].index });

    const zero_one = comptime Query.fromNode(0).out(ir.ANY_RELATION, .{ .min = 0, .max = 1 }).ids();
    var near = try exec.segment(zero_one, &snapshot.view, ctx, .{});
    try expectIds(&near, &.{ nodes[0].index, nodes[1].index, nodes[4].index });

    // {0..0} is the identity.
    const identity = comptime Query.fromNode(2).out(ir.ANY_RELATION, .{ .min = 0, .max = 0 }).ids();
    var same = try exec.segment(identity, &snapshot.view, ctx, .{});
    try expectIds(&same, &.{nodes[2].index});
}
...
test "exec: relation filter expands only matching edges" {
    ...
    var none = try exec.segment(rel1, &snapshot.view, ctx, .{});
    try expectIds(&none, &.{});

    const rel2 = comptime Query.fromNode(4).out(2, .{ .min = 1, .max = 1 }).ids();
    var crossed = try exec.segment(rel2, &snapshot.view, ctx, .{});
    try expectIds(&crossed, &.{nodes[2].index});
    ...
    var via_rel2 = try exec.segment(in_rel2, &snapshot.view, ctx, .{});
    try expectIds(&via_rel2, &.{nodes[4].index});

    const in_rel1 = comptime Query.fromNode(2).in(1, .{ .min = 1, .max = 1 }).ids();
    var via_rel1 = try exec.segment(in_rel1, &snapshot.view, ctx, .{});
    try expectIds(&via_rel1, &.{ nodes[1].index, nodes[5].index });
    ...
    var around = try exec.segment(both_any, &snapshot.view, ctx, .{});
    try expectIds(&around, &.{ nodes[1].index, nodes[3].index, nodes[4].index, nodes[5].index });
}
...
test "exec: set ops against params honor pipeline order" {
    ...
    var difference = try exec.segment(minus, &snapshot.view, ctx, .{ .sets = &.{
        &.{ nodes[0].index, nodes[1].index },
        &.{ nodes[1].index, nodes[2].index },
    } });
    try expectIds(&difference, &.{nodes[0].index});

    const intersect = comptime Query.fromParam(0).intersectParam(1).ids();
    var common = try exec.segment(intersect, &snapshot.view, ctx, .{ .sets = &.{
        &.{ nodes[0].index, nodes[1].index, nodes[2].index },
        &.{ nodes[1].index, nodes[2].index, nodes[3].index },
    } });
    try expectIds(&common, &.{ nodes[1].index, nodes[2].index });

    const join = comptime Query.fromParam(0).unionParam(1).ids();
    var merged = try exec.segment(join, &snapshot.view, ctx, .{ .sets = &.{
        &.{nodes[0].index},
        &.{nodes[7].index},
    } });
    try expectIds(&merged, &.{ nodes[0].index, nodes[7].index });
}
...
test "exec: terminals — ids sorted, count, exists, degree filter" {
    ...
    var sorted = try exec.segment(ids_plan, &snapshot.view, ctx, .{ .sets = &.{
        &.{ nodes[3].index, nodes[0].index, nodes[3].index, nodes[1].index },
    } });
    try expectIds(&sorted, &.{ nodes[0].index, nodes[1].index, nodes[3].index });

    const count_plan = comptime Query.allNodes().count();
    var total = try exec.segment(count_plan, &snapshot.view, ctx, .{});
    ...
    var empty = try exec.segment(exists_plan, &snapshot.view, ctx, .{});
    ...
    var hubs = try exec.segment(hubs_plan, &snapshot.view, ctx, .{});
    try expectIds(&hubs, &.{nodes[5].index});
}
...
test "exec: emit_edges returns in-set edges with prop rows" {
    ...
    var result = try exec.segment(plan, &snapshot.view, ctx, .{ .sets = &.{
        &.{ nodes[0].index, nodes[1].index, nodes[2].index },
    } });
    defer result.deinit(testing.allocator);
...
test "exec: emit_csr builds the induced subgraph" {
    ...
    try testing.expectEqualSlices(u32, &.{nodes[1].index}, try csr.outNeighbors(nodes[0]));
    try testing.expectEqualSlices(u32, &.{nodes[2].index}, try csr.outNeighbors(nodes[1]));
    try testing.expectEqual(@as(usize, 0), (try csr.outNeighbors(nodes[2])).len);
    // Non-members read as not live (empty offsets, membership bitmap).
    try testing.expect(!csr.isLive(nodes[5]));
    try testing.expectError(error.InvalidNode, csr.outNeighbors(nodes[5]));
}
...
test "exec: param binding rejects mismatched count and invalid ids" {
    ...
    try testing.expectError(error.ParamCountMismatch, exec.segment(plan, &snapshot.view, ctx, .{ .sets = &.{
        &.{fixture.nodes[0].index},
    } }));

    const single = comptime Query.fromParam(0).ids();
    try testing.expectError(error.InvalidNode, exec.segment(single, &snapshot.view, ctx, .{ .sets = &.{
...
test "exec: removed param ids drop silently; removed seeds error" {
    ...
    var live_only = try exec.segment(plan, &snapshot.view, ctx, .{ .sets = &.{
    ...
    try testing.expectError(error.InvalidNode, exec.segment(seed_removed, &snapshot.view, ctx, .{}));
}
...
test "exec: a running query is isolated from later mutations" {
    ...
    var still_isolated = try exec.segment(out_of_7, &snapshot.view, ctx, .{});
    try expectIds(&still_isolated, &.{});

    const in_of_0 = comptime Query.fromNode(0).in(ir.ANY_RELATION, .{ .min = 1, .max = 1 }).ids();
    var unchanged = try exec.segment(in_of_0, &snapshot.view, ctx, .{});
...
test "parser: a parsed query executes identically to its builder twin" {
    ...
    var from_text = try exec.segment(parsed.plan(), &snapshot.view, ctx, .{ .sets = &sets });
    defer from_text.deinit(testing.allocator);

    const twin = comptime Query.fromParam(0).out(1, .{ .min = 1, .max = 2 }).minusParam(1).ids();
    var from_builder = try exec.segment(twin, &snapshot.view, ctx, .{ .sets = &.{ &seeds, &blocked } });
