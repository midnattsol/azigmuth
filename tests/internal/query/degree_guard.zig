...
test "degree guard: addEdge with degree_fwd at max returns DegreeLimitReached" {
    ...
    try testing.expectError(error.DegreeLimitReached, graph.addEdge(src, dst, 0, 0));
}

test "degree guard: addEdge with degree_rev at max returns DegreeLimitReached" {
    var graph = try graph_mod.Graph.init(testing.allocator);
    defer graph.deinit();
