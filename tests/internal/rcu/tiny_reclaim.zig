...
test "rcu: tiny slot churn stays bounded under retire + reclaim" {
    ...
    try testing.expect(graph.graph.tiny_fwd_slot_count < 64);
    try testing.expect(graph.graph.tiny_rev_slot_count < 64);
}
...
test "rcu: retired tiny slot is not reused while a reader is pinned" {
    ...
    const before_count = graph.graph.tiny_fwd_slot_count;
    ...
    try testing.expect(graph.graph.tiny_fwd_slot_count >= before_count);
