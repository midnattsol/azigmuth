...
test "rcu: iterator keeps old snapshot while source adjacency mutates" {
    ...
    const old_neighbors = try graph_mod.materializeConsuming(&old_iterator, testing.allocator);
    ...
    const new_neighbors = try graph_mod.materializeConsuming(&new_iterator, testing.allocator);
...
test "rcu: materialize releases reader guard" {
    ...
    try testing.expectEqual(@as(u32, 1), graph.graph.active_readers.load(.acquire));
    const neighbors = try graph_mod.materializeConsuming(&iterator, testing.allocator);
