...
test "graph: debugValidate detects removed node with outgoing adjacency" {
    ...
    helpers.setPublishedFwdDegree(removed_raw, @as(u22, @intCast(1)));
    ...
    helpers.setPublishedRevDegree(live_raw, @as(u22, @intCast(1)));
    graph.graph.edge_count.store(1, .release);
...
test "graph: rejected non-tail removal does not allocate or retire blocks" {
    ...
    const retired_fwd_before = graph.graph.retired_blocks_fwd.items.len;
    const retired_rev_before = graph.graph.retired_blocks_rev.items.len;
    const free_fwd_before = graph.graph.free_blocks_fwd.items.len;
    const free_rev_before = graph.graph.free_blocks_rev.items.len;
    ...
    try testing.expectEqual(retired_fwd_before, graph.graph.retired_blocks_fwd.items.len);
    try testing.expectEqual(retired_rev_before, graph.graph.retired_blocks_rev.items.len);
    try testing.expectEqual(free_fwd_before, graph.graph.free_blocks_fwd.items.len);
    try testing.expectEqual(free_rev_before, graph.graph.free_blocks_rev.items.len);
    try graph.validate();
}
