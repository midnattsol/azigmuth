...
test "remove_node_stress: remove node with outgoing to many destinations" {
    ...
    for (0..target_count) |i| try graph.addEdge(source, targets[i], 0, 0);
...
test "remove_node_stress: remove node with self-edge and many others" {
    ...
    var others: [50]graph_mod.NodeId = undefined;
    for (0..50) |i| others[i] = try graph.addNode();

    try graph.addEdge(node, node, 0, 0);
    for (0..50) |i| try graph.addEdge(node, others[i], 0, 0);
    for (0..50) |i| try graph.addEdge(others[i], node, 0, 0);
...
test "remove_node_stress: remove node with incoming from many nodes" {
    ...
    var senders: [sender_count]graph_mod.NodeId = undefined;
    for (0..sender_count) |i| senders[i] = try graph.addNode();
    for (0..sender_count) |i| try graph.addEdge(senders[i], target, 0, 0);
...
test "remove_node_stress: edgeCount accuracy after removeNode" {
    ...
    const nodes: [10]graph_mod.NodeId = .{ try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode(), try graph.addNode() };
...
test "remove_node_stress: removeNode then addEdge to survivor works" {
    ...
    try testing.expectError(error.EdgeAlreadyExists, graph.addEdge(a, c, 0, 0));
    try graph.validate();
    try testing.expectEqual(@as(u64, 1), graph.edgeCount());
}
