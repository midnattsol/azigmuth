...
test "lifecycle: deinitChecked fails with active repairer" {
    var graph = try graph_mod.Graph.init(testing.allocator);
    defer graph.deinit();

    _ = graph.graph.active_repairers.fetchAdd(1, .acquire);
    defer _ = graph.graph.active_repairers.fetchSub(1, .release);
